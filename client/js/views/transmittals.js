  ...
  function openForm(rec, page) {
    ...
    const schoolField = lockSchool
      ? `<div class="px-3 py-2 text-sm bg-ink-50 border border-ink-100 rounded-lg text-ink-700 font-medium flex items-center gap-2">
           ...
           <input type="hidden" id="f_school" value="${UI.esc(initialSchool)}">
         </div>`
      : UI.input('f_school', { value: initialSchool, placeholder: 'e.g. Agripino Alvarez ES' });

    const body = `
      <div class="space-y-5">
        ${UI.fieldGroup('School Information', UI.row2(
          UI.field('School Name', schoolField, lockSchool ? 'Locked to your assigned school' : null, true),
